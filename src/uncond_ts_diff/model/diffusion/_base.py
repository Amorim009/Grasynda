...
from src.uncond_ts_diff.utils import extract
...
class TSDiffBase(pl.LightningModule):
    ...
    @staticmethod
    def _unpack_scaler_output(scaler_output):
        """Handle gluonts scaler API differences across versions.

        Older versions return (scaled, scale), newer versions may return
        (scaled, loc, scale).
        """
        if not isinstance(scaler_output, tuple):
            return scaler_output, None
        if len(scaler_output) == 2:
            return scaler_output
        if len(scaler_output) >= 3:
            return scaler_output[0], scaler_output[-1]
        raise ValueError("Unexpected scaler output format.")
    ...
    def training_step(self, data, idx):
        ...
        if isinstance(data, dict):
            ...
            x, _ = self._unpack_scaler_output(
                self.scaler(data, torch.ones_like(data))
            )
            features = None
