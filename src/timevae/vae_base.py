...
class BaseVariationalAutoencoder(Model, ABC):
    """
    Mask-aware AE-style base model adapted from VaryTimeAE.

    We intentionally train in reconstruction-only mode here:
    - masked reconstruction loss on observed points
    - KL disabled
    - decoder driven by z_mean during train/test/predict
    """
    ...
    def _get_reconstruction_loss(self, X, X_recons, sample_weight=None):
        ...
        if sample_weight is None:
            per_sample = tf.reduce_mean(err, axis=[1, 2])
            return tf.reduce_mean(per_sample)

        mask = tf.cast(sample_weight, err.dtype)
        mask = mask[..., None]
        num = tf.reduce_sum(err * mask, axis=[1, 2])
        den = tf.reduce_sum(mask, axis=[1, 2]) + 1e-8
        per_sample = num / den
        return tf.reduce_mean(per_sample)
