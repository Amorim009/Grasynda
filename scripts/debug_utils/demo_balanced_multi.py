...
def demonstrate_multi_series(dataset_name='M3', group='Monthly', n_series=5):
    print(f"Demonstrating Balanced Input on {n_series} series...")
    ...
    # Process each series
    output_dir = 'assets/balanced_multi_demo'
    ...
    for idx, uid in enumerate(selected_uids):
        ...
        axes[0].plot(base_ts['ds'], base_ts['y'], color='blue', linestyle='--', label='Baseline Synth', lw=1.5, alpha=0.7)
        ...
        axes[1].plot(bal_ts['ds'], bal_ts['y'], color='red', label='Balanced Synth', lw=2)
...
if __name__ == "__main__":
    demonstrate_multi_series(n_series=5)
