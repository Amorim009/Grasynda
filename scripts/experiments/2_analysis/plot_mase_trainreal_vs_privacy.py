"""
Plot: Average Rank in MASE (Train+Real) vs Average Rank in Privacy (PyMDMA)
...
# ---- Load Train+Real MASE results for NHITS, MLP, KAN ----
trainreal_datasets = ['M3 - Monthly', 'M3 - Quarterly', 'Tourism - Monthly', 'Tourism - Quarterly']
model_names = ['NHITS', 'MLP', 'KAN']

all_model_ranks = []

for model_name in model_names:
    trainreal_path = os.path.join(project_root, "assets", "results", "universal_experiments", f"{model_name}_Train+Real_Results.csv")
    trainreal_df = pd.read_csv(trainreal_path)
    
    trainreal_df = trainreal_df[trainreal_df['Dataset_Full'].isin(trainreal_datasets)]
    trainreal_df = trainreal_df.drop(columns=['Jittering', 'SeasonalMBB'], errors='ignore')
    
    trainreal_long = trainreal_df.melt(id_vars='Dataset_Full', var_name='Method', value_name='MASE')
    
    # Rank MASE per dataset (lower MASE = rank 1)
    mase_ranks = []
    for ds, ds_df in trainreal_long.groupby('Dataset_Full'):
...
print("Train+Real MASE Ranks by Model:")
print(combined_ranks.round(2).to_string())
print()

# ---- Map method names between Train+Real and PyMDMA ----
name_map = {
    'DBA': 'DBA',
    'TSMixup': 'TSMixup',
...
avg_mase_rank = avg_mase_rank.copy()
...
plt.tight_layout()
out_path = os.path.join(project_root, "assets", "results", "pymdma_metrics", "corrected", "mase_trainreal_vs_privacy_rank.png")
